-- Final Mount Atin Mod Menu (Tanpa Fly & Wallhack)
-- Mobile Friendly, Dragable GUI, Hide → Logo, AutoSummit, Manual Teleport

local player = game.Players.LocalPlayer

-- ===== 1. Checkpoint Table (Urut) =====
local checkpoints = {
...
-- Logo (tampil pas hide)
local logo = Instance.new("ImageButton", gui)
logo.Size = UDim2.new(0,50,0,50)
logo.Position = UDim2.new(0.9,0,0.1,0)
logo.Image = "rbxassetid://YOUR_DRAGON_IMAGE_ID" -- ganti ID logo naga
logo.Visible = false
logo.Active = true
logo.Draggable = true
    ...
    if autoSummitActive then
        spawn(function()
            for i = 1, #checkpoints do
                ...
                wait(1) -- nunggu checkpoint injek
            end
