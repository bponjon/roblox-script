...
local targetY = rootPart.Position.Y -- posisi Y saat mulai flying
...
runService.RenderStepped:Connect(function(delta)
    if flying then
        ...
        -- Horizontal: pakai Humanoid:Move() agar animasi jalan normal
        humanoid:Move(moveDir, false)

        -- Vertical: naik/turun sesuai tombol, kalau tidak ditekan tetap targetY
        if verticalSpeed ~= 0 then
            targetY = rootPart.Position.Y + verticalSpeed * delta
        end
