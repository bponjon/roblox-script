-- MountBingung AutoWalk Recorder (final) — client-only loadstring
-- Records HRP CFrame + yaw + jump flag with timestamps. Playback teleports to start then lerps smoothly.
-- Execute from HP/executor (LocalScript). Client-only: safe, doesn't change server state.
...
-- CONFIG
local SAMPLE_INTERVAL_DEFAULT = 0.12   -- seconds between recorded samples (smaller -> smoother & heavier)
local LERP_STEPS_PER_SAMPLE = 6        -- how many sub-steps to lerp between samples (smoothness)
...
local playing = false
local path = {} -- { {t = sec, cf = {x,y,z,rotY}, jump = bool} }
local recordStart = 0
local humanoid, hrp
local recordConn
...
title.Text = "MountBingung — AutoWalk Recorder"
...
icon.Active = true
icon.Draggable = true
icon.Image = "" -- kosong, user bisa set
...
local function recordSample()
    ...
    -- jump was flagged by event or velocity
...
-- playback: teleport to start then lerp between samples
local function playPath()
    ...
    -- teleport to first sample safely
    local first = path[1]
    if first then
        pcall(function()
    ...
    for i = 1, #path do
        ...
        if nextNode then
            -- lerp from cur -> next over sampleInterval using substeps
            local steps = LERP_STEPS_PER_SAMPLE
            local stepTime = (nextNode.t - cur.t) / math.max(1, steps)
            -- ensure positive
            if stepTime <= 0 then stepTime = sampleInterval / math.max(1, steps) end
            for s = 1, steps do
                if not playing then break end
                refreshRefs()
                ...
                local target = Vector3.new(
                    cur.p[1] + (nextNode.p[1] - cur.p[1]) * alpha,
                    cur.p[2] + (nextNode.p[2] - cur.p[2]) * alpha,
                    cur.p[3] + (nextNode.p[3] - cur.p[3]) * alpha
                )
                -- compute yaw interpolation (simple)
                local yawA = cur.yaw or 0
                local yawB = nextNode.yaw or yawA
                -- normalize shortest angle
                local diff = yawB - yawA
                if diff > math.pi then diff = diff - 2*math.pi end
                if diff < -math.pi then diff = diff + 2*math.pi end
                local yawNow = yawA + diff * alpha

                pcall(function()
                    hrp.CFrame = CFrame.new(target) * CFrame.Angles(0, yawNow, 0)
                end)

                -- if jump flag in either sample, try to trigger at start of segment
                if cur.jump then
                ...
                local waited = 0
                -- small wait to simulate time progression
                local waitTime = stepTime
                while waited < waitTime and playing do
                    waited = waited + 0.03
                    wait(0.03)
                end
            end
        else
            -- last node: move directly to it smoothly
            refreshRefs()
            if hrp then
                for n=1,6 do
                    if not playing then break end
                    local target = Vector3.new(cur.p[1], cur.p[2], cur.p[3])
                    pcall(function() hrp.CFrame = hrp.CFrame:Lerp(CFrame.new(target) * CFrame.Angles(0, cur.yaw or 0, 0), 0.5) end)
                    wait(0.04)
                end
            end
        end
...
local function deleteRange(fromIdx, toIdx)
    ...
    local new = {}
    for i=1,#path do
        if i < fromIdx or i > toIdx then table.insert(new, path[i]) end
    end
    path = new
...
btnStop.MouseButton1Click:Connect(function()
    if recording then stopRecord(); btnRecord.Text = "Start Record"; btnRecord.BackgroundColor3 = Color3.fromRGB(170,0,170) end
...
-- optional: stop playback on manual input
if STOP_ON_MANUAL_INPUT then
    UserInputService.InputBegan:Connect(function(input, gp)
        if gp then return end
        if playing and (input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.Keyboard or input.UserInputType == Enum.UserInputType.MouseButton1) then
            stopPlay()
...
-- initial refresh
refreshRefs()
print("MountBingung AutoWalk Recorder (final) loaded. Tips: walk smooth while recording; stop to flush final sample.")
